...
from tensorflow.keras.layers import Input, Dense, Dropout, Conv2D, MaxPooling2D, GlobalAveragePooling2D, Reshape, multiply
...
def se_block(input_tensor, ratio=4):
    ...
    # Excitation: 降维与升维，学习通道权重
    se = Dense(filters // ratio, activation='relu', kernel_initializer='he_normal', use_bias=False)(se)
    se = Dense(filters, activation='sigmoid', kernel_initializer='he_normal', use_bias=False)(se)
    
    # Scale: 将学习到的权重乘回原始特征图
    return multiply([input_tensor, se])


# 4. 构建 SE-CNN 架构
def build_model(num_labels):
    # 使用 Keras 函数式 API 构建带有分支的注意力网络
    inputs = Input(shape=(Config.n_mfcc, Config.max_pad_len, 1))

    # 第 1 组卷积 + SE 模块
    x = Conv2D(16, (2, 2), padding='same', activation='relu')(inputs)
    x = se_block(x)  # 加入通道注意力
    ...
    # 全局池化 (大幅减少参数，策划书中明确提到替代 Flatten)
...
def predict_unlabeled_test(model, le, save_dir):
    ...
    print("无标签预测完成，结果已保存至 test_predictions.csv")
...
if __name__ == "__main__":
    ...
    # 保存原始索引用于计算权重
    y_train_indices = le.transform(y_train_raw)

    y_train = to_categorical(y_train_indices, num_classes=num_labels)
    ...
    history = model.fit(
        ...
        class_weight=weight_dict,   # 传入类别权重
        verbose=1
    )
    ...
    print(f"\n🎉 训练完成！最终 SE-CNN 综合 F1: {final_f1:.4f}")