...
exports.ConversationSummaryTool = void 0;
...
const ConversationSummarySchema = zod_1.z.object({
    userId: zod_1.z.string().describe('用户或宝宝的唯一标识符'),
    babyId: zod_1.z.string().describe('宝宝的唯一标识符'),
    conversationHistory: zod_1.z.array(zod_1.z.object({
        role: zod_1.z.string().describe('消息角色 (user 或 assistant)'),
        content: zod_1.z.string().describe('消息内容'),
    })).describe('要总结的对话历史'),
    summaryType: zod_1.z.enum(['daily', 'weekly', 'monthly']).describe('摘要类型'),
});
/**
 * 对话摘要生成工具
 ...
 * 支持日、周、月多个维度的总结
 */
class ConversationSummaryTool extends tools_1.StructuredTool {
    constructor() {
        super();
        this.name = 'conversation_summary';
        this.description = '总结过往对话历史，提取宝宝行为规律和家长偏好，存入长期记忆库。支持日摘要、周统计、月总结等多个维度。';
        this.schema = ConversationSummarySchema;
    ...
    async _call({ userId, babyId, conversationHistory, summaryType, }) {
        try {
            logger_1.logger.info(`开始生成${summaryType}摘要 [用户: ${userId}, 宝宝: ${babyId}, 对话条数: ${conversationHistory.length}]`);
            // 构建对话总结提示词
            ...
            logger_1.logger.info(`${summaryType}摘要已生成并保存 [用户: ${userId}]`);
            return `成功生成并保存${summaryType}摘要。\n摘要内容：\n${summary}`;
        }
        catch (error) {
            logger_1.logger.error('生成对话摘要失败:', error);
     ...
     * 构建摘要生成提示词
     */
    buildSummaryPrompt(conversationText, type) {
...
exports.ConversationSummaryTool = ConversationSummaryTool;
//# sourceMappingURL=conversationSummaryTool.js.map