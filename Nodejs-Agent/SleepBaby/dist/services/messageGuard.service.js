"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.messageGuardService = exports.MessageGuardService = void 0;
const DEFAULT_SENSITIVE_WORDS = ['傻逼', '妈的', '操', '去死'];
const splitWords = (words) => words
    .split(',')
    .map((word) => word.trim())
    .filter(Boolean);
class MessageGuardService {
    constructor(words = []) {
        this.sensitiveWords = words.length > 0 ? words : DEFAULT_SENSITIVE_WORDS;
    }
    static fromEnv() {
        const configuredWords = process.env.SENSITIVE_WORDS
            ? splitWords(process.env.SENSITIVE_WORDS)
            : [];
        return new MessageGuardService(configuredWords);
    }
    findSensitiveWords(text) {
        if (!text.trim()) {
            return [];
        }
        const normalizedText = text.toLowerCase();
        return this.sensitiveWords.filter((word) => normalizedText.includes(word.toLowerCase()));
    }
    validateInput(text) {
        const matchedWords = this.findSensitiveWords(text);
        if (matchedWords.length === 0) {
            return { ok: true, matchedWords: [] };
        }
        return {
            ok: false,
            matchedWords,
            message: '输入内容包含敏感词，请调整后再试',
        };
    }
}
exports.MessageGuardService = MessageGuardService;
exports.messageGuardService = MessageGuardService.fromEnv();
//# sourceMappingURL=messageGuard.service.js.map