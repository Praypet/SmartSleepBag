...
export class ShortTermMemory {
   ...
   */
  async storeMemory(userId: string, data: { role: string; content: string }, ttlSeconds: number = 3600) {
    try {
      ...
      logger.info(`成功存储短期记忆 [用户: ${userId}, TTL: ${ttlSeconds}s]`);
    } catch (error) {
      logger.error('存储短期记忆失败:', error);
    }
  }
  ...
  async storePhysiologicalData(userId: string, dataType: string, value: any) {
    try {
      ...
      logger.info(`存储实时数据 [用户: ${userId}, 类型: ${dataType}]`);
    } catch (error) {
      logger.error('存储实时数据失败:', error);
    }
  }
   ...
   * @param userId 用户ID
   */
  async retrieveMemory(userId: string): Promise<Array<{ role: string; content: string }>> {
    try {
      const key = `session:${userId}:history`;
      const data = await this.client.lrange(key, 0, -1);

      const memories = (data || []).map((item: string) => {
        try {
   ...
   * @param dataType 数据类型
   */
  async getPhysiologicalData(userId: string, dataType: string): Promise<any> {
    try {
      const key = `realtime:${userId}:${dataType}`;
  ...
  async clearMemory(userId: string) {
    try {
      ...
      logger.info(`已清除用户 ${userId} 的对话历史`);
    } catch (error) {
      logger.error('清除短期记忆失败:', error);
    }
  }
}
