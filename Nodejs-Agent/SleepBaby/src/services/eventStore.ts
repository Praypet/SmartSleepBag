...
export class EventStore {
  ...
  private async initTables() {
    ...
    for (const table of tables) {
      try {
        ...
        logger.error('初始化表失败:', error);
      }
    }
    logger.info('事件存储表已初始化');
  }

  /**
   * 存储宝宝基本信息
   */
  async storeBabyProfile(babyId: string, profile: {
    name: string;
    gender: string;
    birth_date: string;
    avatar_url?: string;
    ...
    try {
      ...
      logger.info(`宝宝信息已保存 [ID: ${babyId}]`);
    } catch (error) {
      logger.error('保存宝宝信息失败:', error);
    }
  }
   ...
   * 存储睡眠记录
   */
  async storeSleepRecord(babyId: string, userId: string, record: {
    date: string;
    ...
    try {
      ...
      logger.info(`睡眠记录已保存 [宝宝: ${babyId}, 日期: ${record.date}]`);
    } catch (error) {
      logger.error('保存睡眠记录失败:', error);
    }
  }
   ...
   * 存储生理事件
   */
  async storeHealthEvent(babyId: string, userId: string, event: {
    event_type: string;
    temperature?: number;
    heart_rate?: number;
    ...
    try {
      ...
      logger.info(`生理事件已记录 [宝宝: ${babyId}, 类型: ${event.event_type}]`);
    } catch (error) {
      logger.error('记录生理事件失败:', error);
    }
  }
   ...
   * 存储日常事件
   */
  async storeEventLog(babyId: string, userId: string, event: {
    event_type: string;
    description: string;
    data?: any;
  }) {
    ...
    try {
      ...
      logger.info(`事件已记录 [宝宝: ${babyId}, 类型: ${event.event_type}]`);
    } catch (error) {
   ...
   * 存储/更新用户配置
   */
  async storeUserConfig(userId: string, babyId: string, config: any) {
    const query = `
   ...
   * 查询睡眠记录
   */
  async getSleepRecords(babyId: string, days: number = 30): Promise<any[]> {
    const query = `
   ...
   * 按日期查询睡眠记录。
   */
  async getSleepRecordsByDate(babyId: string, date: string): Promise<any[]> {
    const query = `
      SELECT * FROM sleep_records
    ...
    try {
      ...
      logger.error('按日期查询睡眠记录失败:', error);
      return [];
    }
  }
   ...
   * 查询生理事件
   */
  async getHealthEvents(babyId: string, days: number = 7): Promise<any[]> {
    const query = `
   ...
   * 按日期查询生理事件。
   */
  async getHealthEventsByDate(babyId: string, date: string): Promise<any[]> {
    const query = `
      SELECT * FROM health_events
    ...
    try {
      ...
      logger.error('按日期查询生理事件失败:', error);
      return [];
    }
  }
   ...
   * 查询事件日志。
   */
  async getEventLogs(babyId: string, days: number = 7): Promise<any[]> {
    const query = `
   ...
   * 按日期查询事件日志。
   */
  async getEventLogsByDate(babyId: string, date: string): Promise<any[]> {
    const query = `
      SELECT * FROM event_logs
      WHERE baby_id = $1 AND DATE(created_at) = $2
      ORDER BY created_at DESC
    ...
    try {
      ...
      logger.error('按日期查询事件日志失败:', error);
      return [];
    }
  }
}
